-- First, drop the existing check constraint and create a new one that includes all existing types plus 'books'
ALTER TABLE products DROP CONSTRAINT IF EXISTS products_product_type_check;

-- Add a new constraint that includes all existing types plus 'books'
ALTER TABLE products ADD CONSTRAINT products_product_type_check 
CHECK (product_type IN ('single', 'bundle', 'variable', 'books'));
